...
class IodineVPNService(dbus.service.Object):
    ...
    def _emit_ip4_config(self):
        ...
        prefix_val = 27
        client_ip_u32 = dbus.UInt32(ip4_to_uint32_network(self.client_ip_str))
        server_ip_u32 = dbus.UInt32(ip4_to_uint32_network(self.server_ip_str))
        prefix_u32 = dbus.UInt32(prefix_val)
        mtu_u32 = dbus.UInt32(mtu_val)
        gateway_ip_str = self.external_ip_str or self.server_ip_str
        gateway_u32_host = dbus.UInt32(ip4_to_uint32_host(gateway_ip_str))
        gateway_u32_net = dbus.UInt32(ip4_to_uint32_network(gateway_ip_str))

        route = dbus.Struct([
            dbus.UInt32(ip4_to_uint32_network("10.0.0.0")),
            dbus.UInt32(27),
            gateway_u32_net
        ])

        ip4config = {
            "gateway": gateway_u32_host,
            "address": client_ip_u32,
            "prefix": prefix_u32,
